...
<!-- start header -->
    ...
    <div class="searchbox">
		...
		<blockquote>查询条件: 公司,联系人,手机,类型,来源,等级,员工</blockquote>
   	  </form>
    </div>
    <!-- end searchbox -->
    ...
    <div class="page">
            ...
            <div class="simplebox">
           	 ...
           	 <ul class="list-menu">
							...
							ELSE
							rs.Open "Select top "&intPageSize&" * From [client] where cYn = 1 "&sql&" and cid < ( SELECT Min(cid) FROM ( SELECT TOP "&pagenum&" cid FROM [client]  where cYn = 1 "&sql&" ORDER BY cId desc ) AS T ) Order By cId Desc ",conn,1,1
							END IF
							Set Rsstr=conn.Execute("Select count(cid) As RecordSum From [client] where cYn = 1 "&sql&" ",1,1)
            ...
            <%=pagelist("Listall.asp?Keyword="&Keyword&"", PN,TotalPages,TotalRecords)%>
