    ...
    <div id="columns">
            ...
            <li class="widget color-2">  
                ...
                <div class="widget-content">
					<ul>
						<%
						Set rs1 = Server.CreateObject("ADODB.Recordset")
						rs1.Open "Select Top 5 * From [OA_mms_Receive] WHERE ( oIsread = 0 and oReceiver = '"&Session("CRM_name")&"' and oAttime is null ) or ( oAttime is not null and oAttime < Now()+ 0.007 and oIsread = 0 and oReceiver = '"&Session("CRM_name")&"' ) Order By Id Desc",conn,3,1
						i=0
						If rs1.RecordCount > 0 Then
						If Not rs1.Eof Then 
						do while not (rs1.eof or err)
                ...
                <div class="widget-content">
					<ul>
						...
						Set rs1 = Server.CreateObject("ADODB.Recordset")
							...
							rs1.Open "Select Top 5 * From Records where 1=1 and rNextTime + 3 >= Now() and rNextTime - 3 <= Now() Order By rNextTime Desc",conn,3,1
							else
							rs1.Open "Select Top 5 * From Records where rUser='" & Session("CRM_name") & "' and rNextTime + 3 >= Now() and rNextTime - 3 <= Now() Order By rNextTime Desc",conn,3,1
							end if
						i=0
						If rs1.RecordCount > 0 Then
						If Not rs1.Eof Then 
            ...
            <li class="widget color-6">  
                ...
                <div class="widget-content">
					<ul>
						...
						Set rs1 = Server.CreateObject("ADODB.Recordset")
							if Session("CRM_level") = 9 Then
							rs1.Open "Select Top 5 * From Hetong Where hedate + 1 >= Now() Order By hedate asc",conn,3,1
							else
							rs1.Open "Select Top 5 * From Hetong Where hedate + 1 >= Now() and hUser In (" & arrUser & ") Order By hedate asc",conn,3,1
							end if
						i=0
						If rs1.RecordCount > 0 Then
						If Not rs1.Eof Then
        ...
        <ul id="column3" class="column">
					...
					<ul style="padding-top:10px;">
						...
						<style>.td_l_c {border:0;border-bottom:1px solid #ddd;}
							...
							<tr>
								...
								<td class="td_l_c"><%=EasyCrm.getCountItem("Client","cid","cid"," and cYn=1 and cUser = '"&Session("CRM_name")&"' and cDate + 7 >= Now() ")%></td>
								<td class="td_l_c"><%=EasyCrm.getCountItem("Client","cid","cid"," and cYn=1 and cUser = '"&Session("CRM_name")&"' and cDate + 30 >= Now() and cDate + 7 <= Now() ")%></td>
								<td class="td_l_c"><%=EasyCrm.getCountItem("Client","cid","cid"," and cYn=1 and cUser = '"&Session("CRM_name")&"' and cDate + 30 <= Now() ")%></td>
								<td class="td_l_c"><%=EasyCrm.getCountItem("Client","cid","cid"," and cYn=1 and cUser = '"&Session("CRM_name")&"' ")%></td>
							</tr>
							<tr>
								<td class="td_l_c">跟单</td>
								<td class="td_l_c"><%=EasyCrm.getCountItem("Records","rid","rid"," and rUser = '"&Session("CRM_name")&"' and rTime + 7 >= Now() ")%></td>
								<td class="td_l_c"><%=EasyCrm.getCountItem("Records","rid","rid"," and rUser = '"&Session("CRM_name")&"' and rTime + 30 >= Now() and rTime + 7 <= Now() ")%></td>
								<td class="td_l_c"><%=EasyCrm.getCountItem("Records","rid","rid"," and rUser = '"&Session("CRM_name")&"' and rTime + 30 <= Now() ")%></td>
								<td class="td_l_c"><%=EasyCrm.getCountItem("Records","rid","rid"," and rUser = '"&Session("CRM_name")&"' ")%></td>
							</tr>
							<tr>
								<td class="td_l_c">订单</td>
								<td class="td_l_c"><%=EasyCrm.getCountItem("Order","oid","oid"," and oUser = '"&Session("CRM_name")&"' and oTime + 7 >= Now() ")%></td>
								<td class="td_l_c"><%=EasyCrm.getCountItem("Order","oid","oid"," and oUser = '"&Session("CRM_name")&"' and oTime + 30 >= Now() and oTime + 7 <= Now() ")%></td>
								<td class="td_l_c"><%=EasyCrm.getCountItem("Order","oid","oid"," and oUser = '"&Session("CRM_name")&"' and oTime + 30 <= Now() ")%></td>
								<td class="td_l_c"><%=EasyCrm.getCountItem("Order","oid","oid"," and oUser = '"&Session("CRM_name")&"' ")%></td>
							</tr>
							<tr>
								<td class="td_l_c">合同</td>
								<td class="td_l_c"><%=EasyCrm.getCountItem("Hetong","hid","hid"," and hUser = '"&Session("CRM_name")&"' and hTime + 7 >= Now() ")%></td>
								<td class="td_l_c"><%=EasyCrm.getCountItem("Hetong","hid","hid"," and hUser = '"&Session("CRM_name")&"' and hTime + 30 >= Now() and hTime + 7 <= Now() ")%></td>
								<td class="td_l_c"><%=EasyCrm.getCountItem("Hetong","hid","hid"," and hUser = '"&Session("CRM_name")&"' and hTime + 30 <= Now() ")%></td>
								<td class="td_l_c"><%=EasyCrm.getCountItem("Hetong","hid","hid"," and hUser = '"&Session("CRM_name")&"' ")%></td>
							</tr>
							<tr>
								<td class="td_l_c" style="border:0;">售后</td>
								<td class="td_l_c" style="border:0;"><%=EasyCrm.getCountItem("Service","sid","sid"," and sUser = '"&Session("CRM_name")&"' and sTime + 7 >= Now() ")%></td>
								<td class="td_l_c" style="border:0;"><%=EasyCrm.getCountItem("Service","sid","sid"," and sUser = '"&Session("CRM_name")&"' and sTime + 30 >= Now() and sTime + 7 <= Now() ")%></td>
								<td class="td_l_c" style="border:0;"><%=EasyCrm.getCountItem("Service","sid","sid"," and sUser = '"&Session("CRM_name")&"' and sTime + 30 <= Now() ")%></td>
								<td class="td_l_c" style="border:0;"><%=EasyCrm.getCountItem("Service","sid","sid"," and sUser = '"&Session("CRM_name")&"' ")%></td>
							</tr>
							<%else%>
							<tr>
								<td class="td_l_c">客户</td>
								<td class="td_l_c"><%=EasyCrm.getCountItem("Client","cid","cid"," and cYn=1 and cDate + 7 >= Now() ")%></td>
								<td class="td_l_c"><%=EasyCrm.getCountItem("Client","cid","cid"," and cYn=1 and cDate + 30 >= Now() and cDate + 7 <= Now() ")%></td>
								<td class="td_l_c"><%=EasyCrm.getCountItem("Client","cid","cid"," and cYn=1 and cDate + 30 <= Now() ")%></td>
								<td class="td_l_c"><%=EasyCrm.getCountItem("Client","cid","cid"," and cYn=1 ")%></td>
							</tr>
							<tr>
								<td class="td_l_c">跟单</td>
								<td class="td_l_c"><%=EasyCrm.getCountItem("Records","rid","rid"," and rTime + 7 >= Now() ")%></td>
								<td class="td_l_c"><%=EasyCrm.getCountItem("Records","rid","rid"," and rTime + 30 >= Now() and rTime + 7 <= Now() ")%></td>
								<td class="td_l_c"><%=EasyCrm.getCountItem("Records","rid","rid"," and rTime + 30 <= Now() ")%></td>
								<td class="td_l_c"><%=EasyCrm.getCountItem("Records","rid","rid"," ")%></td>
							</tr>
							<tr>
								<td class="td_l_c">订单</td>
								<td class="td_l_c"><%=EasyCrm.getCountItem("Order","oid","oid"," and oTime + 7 >= Now() ")%></td>
								<td class="td_l_c"><%=EasyCrm.getCountItem("Order","oid","oid"," and oTime + 30 >= Now() and oTime + 7 <= Now() ")%></td>
								<td class="td_l_c"><%=EasyCrm.getCountItem("Order","oid","oid"," and oTime + 30 <= Now() ")%></td>
								<td class="td_l_c"><%=EasyCrm.getCountItem("Order","oid","oid"," ")%></td>
							</tr>
							<tr>
								<td class="td_l_c">合同</td>
								<td class="td_l_c"><%=EasyCrm.getCountItem("Hetong","hid","hid"," and hTime + 7 >= Now() ")%></td>
								<td class="td_l_c"><%=EasyCrm.getCountItem("Hetong","hid","hid"," and hTime + 30 >= Now() and hTime + 7 <= Now() ")%></td>
								<td class="td_l_c"><%=EasyCrm.getCountItem("Hetong","hid","hid"," and hTime + 30 <= Now() ")%></td>
								<td class="td_l_c"><%=EasyCrm.getCountItem("Hetong","hid","hid"," ")%></td>
							</tr>
							<tr>
								<td class="td_l_c" style="border:0;" >售后</td>
								<td class="td_l_c" style="border:0;"><%=EasyCrm.getCountItem("Service","sid","sid"," and sTime + 7 >= Now() ")%></td>
								<td class="td_l_c" style="border:0;"><%=EasyCrm.getCountItem("Service","sid","sid"," and sTime + 30 >= Now() and sTime + 7 <= Now() ")%></td>
								<td class="td_l_c" style="border:0;"><%=EasyCrm.getCountItem("Service","sid","sid"," and sTime + 30 <= Now() ")%></td>
            ...
            <li class="widget color-8">  
                <div class="widget-head">
                    <h3>生日提醒 <span class='remind'><%=EasyCrm.getCountItem("Linkmans","lid","lid"," and lBirthday = #"&date()&"# ")%></span>　</h3>
                </div>
                <div class="widget-content">
					<ul>
					<%
					...
					i=i+1
					%>
						<li <%if i=rs.recordcount then%>class='none'<%end if%> ><span class="r"><%=rs("lName")%>&nbsp; <%=EasyCrm.FormatDate(rs("lBirthday"),10)%></span><img src="<%=SiteUrl&skinurl%>images/ico/sr.gif">&nbsp; <a onclick='Client_InfoView<%=rs("cId")%>()' style="cursor:pointer" ><%=EasyCrm.getNewItem("Client","cId",rs("cId"),"cCompany")%></a></li>
