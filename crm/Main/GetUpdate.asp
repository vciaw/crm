...
%>
	...
	function CheckInput()
	{
		...
		if (<%=Must_Client_cType%>=="1"){if(document.all.Type.value == ""){art.dialog({title: 'Error',time: 1,icon: 'warning',content: '<%=L_Client_cType & alert04%>'});document.all.Type.focus();return false;}}
					...
					<td valign="top" class="td_n pdl10 pdr10 pdt10"> 
						...
						<col width="120" /><col width="300" /><col width="120" />
							...
							<tr> 
								...
								<td class="td_r_l"> <%=EasyCrm.getNewItem("Client","cID",""&cID&"","cTrade")%>&nbsp;&nbsp;<%=EasyCrm.getNewItem("Client","cID",""&cID&"","cStrade")%></td>
